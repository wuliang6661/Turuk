...
//                deltaY = deltaY < 0 ? 0 : (deltaY > mHeaderView.getHeight() ? mHeaderView.getHeight() : deltaY);
                ...
                if (deltaY > 0 && deltaY >= getScrollY() && mIsLayout) {
                    ...
                    if (mContentTop <= mHeaderCurBottom) {
                        //头部下拉是的动画
                      //  mHeaderView.layout(mHeadInitRect.left, 0, mHeadInitRect.right, mHeaderCurBottom);
            ...
            case MotionEvent.ACTION_UP: {
                //反弹
                if (mIsMoving) {
                    mHeaderView.layout(mHeadInitRect.left, 0, mHeadInitRect.right, mHeadInitRect.bottom);
                    TranslateAnimation headAnim = new TranslateAnimation(0, 0, 0, 0);
                    ...
                    // 回调监听器
                    if (mContentTop > mContentInitRect.top  && mOnTurnListener != null){
