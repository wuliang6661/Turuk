...
public class TelPhoneUtils {
    ...
    public static void sendMessage(final Context mContext,final String strPhone,final String strMsgContext){
        ...
        if (strMsgContext.length() > 70) {
            ArrayList<String> msgs = smsManager.divideMessage(strMsgContext);
            ArrayList<PendingIntent> sentIntents =  new ArrayList<PendingIntent>();
            for(int i = 0;i<msgs.size();i++){
                sentIntents.add(sendIntent);
            }
            smsManager.sendMultipartTextMessage(strPhone, null, msgs, sentIntents, null);
        } else {
            smsManager.sendTextMessage(strPhone, null, strMsgContext, sendIntent, backIntent);
        }
